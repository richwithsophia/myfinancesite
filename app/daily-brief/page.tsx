  ...
  whatToWatch: [
    {
      ...
      detail:
        "Any hint that the Fed is slowing rate cuts because of tariff-driven inflation could move markets sharply. Watch for the words 'patient' and 'data-dependent' — that's Fed-speak for 'we're not cutting anytime soon.'",
    },
    {
      item: "Friday jobs report (NFP)",
      detail:
        "A strong jobs number could push yields back up and weigh on stocks. A weak number might actually rally the market — because it gives the Fed cover to cut rates. Yes, bad news can be good news in markets. Welcome.",
    },
    {
      item: "Retaliatory tariff announcements",
      detail:
        "Canada has already signaled counter-tariffs. If Mexico follows, expect another volatile session. Keep an eye on consumer staples stocks — they're the canary in the coal mine for price pressure.",
    },
  ],
  tacticalInsight: {
    title: "Don't touch your long-term investments",
    body:
      "If your money is in a diversified index fund for a goal that's 5+ years away, today's drop is noise. The S&P 500 has recovered from every single correction in its history. What you should do: check your cash position. If you don't have 3–6 months of expenses in a high-yield savings account (currently paying ~4.5–5%), a volatile market is a great reminder to build that buffer before adding more to investments.",
  },
};
    ...
    <div className="min-h-screen bg-[#faf8f5] text-[#1a1a1a] font-sans">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#faf8f5]/90 backdrop-blur-md border-b border-[#1a1a1a]/5">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm text-[#1a1a1a]/50 font-medium">
            ...
            <a href="/daily-brief" className="text-emerald-600 font-semibold">Daily Brief</a>
      ...
      <main className="max-w-3xl mx-auto px-6 pt-32 pb-24">
            ...
            {brief.marketPerformance.map((item, i) => (
              <div
                ...
                className={`flex items-center justify-between px-6 py-4 ${
                  i !== brief.marketPerformance.length - 1
        ...
        <section className="mb-10">
          ...
          <div className="flex flex-col gap-4">
            {brief.keyDevelopments.map((item) => (
              <div
                key={item.headline}
                className="bg-white border border-[#1a1a1a]/5 rounded-2xl p-6 hover:border-emerald-200 transition-colors"
              >
          ...
          <div className="bg-white border border-[#1a1a1a]/5 rounded-2xl overflow-hidden">
            {brief.whatToWatch.map((item, i) => (
              <div
                key={item.item}
                className={`px-6 py-5 ${
                  i !== brief.whatToWatch.length - 1 ? "border-b border-[#1a1a1a]/5" : ""
                }`}
              >
        ...
        <div className="bg-white border border-[#1a1a1a]/5 rounded-2xl p-8 text-center">
          ...
          <a
            href="#"
            className="inline-block bg-[#1a1a1a] text-white font-bold px-8 py-3 rounded-full text-sm hover:bg-emerald-700 transition-colors"
          >
